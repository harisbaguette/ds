...
(async () => {
  ...
  async function inspect(name, selector) {
    ...
    const problems = await page.locator(selector).evaluateAll(frames => {
      ...
      if ([...document.images].some(img => !img.complete || !img.naturalWidth)) out.push('image missing');
  ...
  try {
    for (const width of [320, 375, 768, 1101, 1440, 1920]) {
      ...
      for (const style of ['base', 'ink', 'block', 'landscape']) {
      ...
      await inspect(`${width} styles`, '.style-cover');
