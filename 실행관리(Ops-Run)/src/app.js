(() => {
  ...
  const saved = Array.isArray(stored.saved) ? stored.saved.filter(item => item && validPattern(item.id)).map(item => ({ id: item.id, style: 'main' })) : [];
  const state = {
  ...
  function hash(overrides = {}) {
    ...
    if (['patterns', 'system'].includes(next.page) || (next.page === 'saved' && next.detail)) params.set('style', next.style);
  ...
  function navigate(overrides, { replace = false, overlay = false } = {}) {
    if (overlay && state.page !== 'system' && !state.detail) {
  ...
  function showSuggestions() {
    ...
    suggestions = state.page === 'system' ? systemRegistry.matching(query).slice(0, 6) : isCollection() ? library.suggestions(state, query) : catalog.patterns.filter(p => matches(p, query) && (state.page !== 'saved' || state.saved.some(item => item.id === p.id))).slice(0, 6);
    suggestionIndex = -1;
