(() => {
  ...
  const sourceButton = id => `<button class="source-button" data-system-detail="${id}" data-focus="source-${id}" aria-label="${e(r.index.get(id)?.name||id)} 코드 보기" title="코드 보기">${p.icon('arrow')}</button>`;
  ...
  function main(state) {
    ...
    return `<section class="system-workspace" aria-labelledby="system-title"><div class="system-heading"><div><h2 id="system-title" class="sr-only">${state.query ? '“' + e(state.query) + '”' : (state.category === 'all' ? '메인 스타일' : r.sections.find(s => s.id === state.category).name)}</h2></div></div>${state.query ? `<div class="system-query">${items.length}개 부품 <button data-action="clear-query">검색 해제</button></div>` : ''}${items.length ? `<div class="system-board ds theme-${state.style}" data-style="${state.style}"${state.category==='all'&&!state.query?' data-overview':''}>${r.sections.filter(s => s.id !== 'all').sort((a,b)=>['foundations','buttons','fields','selection','navigation','feedback','composition','primitives','page'].indexOf(a.id)-['foundations','buttons','fields','selection','navigation','feedback','composition','primitives','page'].indexOf(b.id)).map(section => {
