      ...
      for (const route of routes) {
        ...
        const result = await page.evaluate(() => {
          ...
          for (const el of controls) {
            ...
            const hitArea = el.matches('input[type="checkbox"], input[type="radio"]') && el.labels?.[0] ? el.labels[0] : el;
          ...
          // Resolve CSS Color 4 (e.g. color-mix -> color(srgb ...)) through the browser.
          ...
          const rgb = value => {
            if (!colorCache.has(value)) {
              ...
              const c = [...colorContext.getImageData(0, 0, 1, 1).data]; c[3] /= 255; colorCache.set(value, c);
            }
