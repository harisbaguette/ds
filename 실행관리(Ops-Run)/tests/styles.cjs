...
(async()=>{
 ...
 try {
  ...
  for(const style of retired){
   ...
   await page.waitForURL('**/#/system?style=main&category=buttons');
   check(style+' 이전 주소는 메인으로 이동',page.url().includes('style=main')&&await page.locator('.specimen').count()===1);
  }
  ...
  await page.goto(origin+'/#/system?style=main');
  const contrast=await page.evaluate(()=>{
   const source=document.querySelector('.system-board'),s=getComputedStyle(source);
  ...
  await page.goto(origin+'/#/system?style=main&category=buttons');
  ...
  for(const width of [320,375,768,1101,1440,1920]) {
   ...
   for(const route of ['system?style=main','dictionary','system?style=main&category=page','system?style=main&detail=button']) {
  ...
  await page.goto(origin+'/#/system?style=main&category=page');await page.screenshot({path:path.join(out,'page.png'),fullPage:true});
  await page.goto(origin+'/#/dictionary');await page.screenshot({path:path.join(out,'dictionary.png')});
  await page.setViewportSize({width:375,height:900});await page.goto(origin+'/#/system?style=main&category=selection');await page.screenshot({path:path.join(out,'mobile.png'),fullPage:true});
