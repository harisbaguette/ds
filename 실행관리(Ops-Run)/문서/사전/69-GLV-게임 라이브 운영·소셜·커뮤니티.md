...
| GLV-104 | 안티치트 설치 안내 — anti-cheat install | 흐름 | 보안 프로그램이 왜 필요한지 알리고 설치 뒤 재시작 안내 | 확인 [QZVANSW] |
| GLV-105 | 안티치트 충돌 프로그램 안내 | 부품 | 같이 못 쓰는 프로그램 이름을 알려 지우게 함 | 확인 [QZVANSW] |
| GLV-106 | PC 보안 설정 요구 안내 | 부품 | 켜야 할 윈도 보안 설정과 업데이트를 알려 줌 | 확인 [QZVANRES] |
| GLV-107 | 기술 차단과 계정 제재 구분 | 기준 | 설정 때문에 막힌 것은 제재와 다른 화면으로 알림 | 확인 [QZVANRES] |
| GLV-108 | 제재 참조 번호 — reference ID | 부품 | 이의 신청에 쓸 제재 번호를 제재 화면에 보여 줌 | 확인 [QZEACAPL] |
| GLV-109 | 이의 신청 창구 안내 | 기준 | 어느 고객지원 창구로 가야 하는지 길을 알려 줌 | 확인 [QZEACAPL] · 대조 [QZRIOTAPL] |
| GLV-110 | 신뢰 점수 매칭 — trust factor | 기준 | 계정 기록과 신고 이력으로 비슷한 사람끼리 묶음 | 확인 [QZCSTRUST] |
| GLV-111 | 매너 등급 — manner grade | 부품 | 최고부터 위험까지 단계로 내 매너를 보여 줌 | 확인 [QZSAMANR] |
| GLV-112 | 매너 등급 오르내림 기준 안내 | 기준 | 욕설·탈주·신고는 내리고 칭찬·완주는 올림 | 확인 [QZSAMANR] |
| GLV-113 | 오래 쉬면 비활성 등급 | 기준 | 30일 넘게 안 들어오면 등급을 비활성으로 | 확인 [QZSAMANR] |
| GLV-114 | 높은 매너 등급 꾸미기 보상 | 부품 | 좋은 등급에 가방·명패 같은 꾸미기를 줌 | 확인 [QZSAMANR] |
| GLV-115 | 칭찬 등급 떨어짐 속도 조절 — endorsement decay | 기준 | 칭찬 등급이 너무 빨리 떨어지지 않게 | 확인 [QZOWENDR] · 대조 [QVLOLHON] |
| GLV-116 | 숨은 행동 점수 — behavior score | 기준 | 신고와 이탈을 점수로 모아 매칭에 반영 | 확장 [MK] |
| GLV-117 | 격리 매칭 — low priority queue | 흐름 | 비매너가 쌓이면 따로 몇 판 하게 함, 남은 판 수 표시 | 확장 [MK] |
| GLV-118 | 행동 요약 알림 — conduct summary | 부품 | 최근 받은 신고와 칭찬을 기간별로 요약 | 확장 [MK] |
| GLV-119 | 프로필 제재 기록 공개 | 기준 | 부정행위 제재 기록을 프로필에 남김, 대상 게임을 분명히 | 확장 [MK] |
| GLV-120 | 크리에이터 코드 적용 표시 | 부품 | 결제 화면에 지금 돕는 크리에이터 이름을 보여 줌 | 확장 [MK] |
| GLV-121 | 크리에이터 코드 만료 알림 | 부품 | 적용 기간이 끝나기 전에 다시 넣으라고 알림 | 확장 [MK] |
| GLV-122 | 없는 크리에이터 코드 바로 알림 | 부품 | 틀린 코드면 입력칸 아래에 바로 알려 줌 | 확장 [MK] |
