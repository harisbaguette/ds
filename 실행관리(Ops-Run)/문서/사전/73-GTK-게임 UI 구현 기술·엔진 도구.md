...
| GTK-155 | 번역 글자 늘어남 여유 — text expansion budget | 기준 | 번역하면 길어질 몫만큼 칸을 넉넉히 비움 | 확인 [QZW3CSIZE] |
| GTK-156 | 짧은 글일수록 더 늘어남 | 기준 | 버튼처럼 짧은 글은 두세 배까지 늘 수 있음 | 확인 [QZW3CSIZE] |
| GTK-157 | 가장 긴 언어로 길이 검사 | 기준 | 독일어처럼 긴 언어로 먼저 넣어 잘림을 찾음 | 확인 [QZUNIEXP] · 대조 [QZW3CSIZE] |
| GTK-158 | 한국어로 만든 칸 주의 | 기준 | 한국어 원문은 영어·독일어로 가면 더 길어짐 | 확인 [QZUNIEXP] |
| GTK-159 | 의사 번역 늘이기 비율 — Expander | 부품 | 가짜 번역을 얼마나 늘릴지 비율로 정함 | 확인 [QZUNIEXP] · 대조 [QTPSEUDO] |
| GTK-160 | 글 길이 따라 늘어나는 칸 — auto-size layout | 구성 | 글이 길면 버튼과 칸이 함께 넓어짐 | 확장 [MK] |
| GTK-161 | 문자열별 최대 글자 수 — max length per string | 부품 | 번역가에게 칸마다 넣을 수 있는 글자 수를 알려 줌 | 확장 [MK] |
| GTK-162 | 잘린 글 전체 보기 | 부품 | 잘린 글은 길게 누르거나 마우스를 올리면 다 보여 줌 | 확장 [MK] |
